...
├── tests/                            # 测试套件（标准库 unittest，零依赖，192 项）
...
```bash
python -m unittest discover -s tests -t .      # 192 项，零依赖
```
...
三态口径修好了「未记录被误判」，但还有一个洞：**你确实做了徒手训练，只是没填字段**。
在「二、今日三件事」写了「做了二十个俯卧撑」，`运动时长_min` 却留空 ——
这天仍然落在「未记录」里。而运动占健康分权重 **0.25**，是全部子项里最重的一项。
...
| `exercise_min` | 运动分钟（正式记录，或由描述折算） |
| `exercise_src` | 运动时长来源：`record`=字段填报 / `derived`=描述折算 / `NULL`=无 |
